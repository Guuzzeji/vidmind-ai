import os
import base64
from scenedetect import open_video, SceneManager, save_images, ContentDetector, AdaptiveDetector
...
def create_clips(vid_path: str, clips_path: str) -> (list, bool):
    ...
    # scene_manager.add_detector(ContentDetector(threshold=16))
    # scene_manager.add_detector(AdaptiveDetector(
    #     adaptive_threshold=4.5, min_scene_len=60, min_content_val=8))
    # ! Stop editing this shit and find a better system
    scene_manager.add_detector(AdaptiveDetector(
        adaptive_threshold=3, min_content_val=13))
    scene_manager.detect_scenes(video)
    scenes = scene_manager.get_scene_list()

    save_images(scenes, video, num_images=3,
